...
export const DashboardPage: React.FC<DashboardPageProps> = ({ students, simulados, corrections, setActivePage }) => {

  const chartData = corrections
    .sort((a, b) => new Date(a.submittedAt).getTime() - new Date(b.submittedAt).getTime())
    .map(c => ({
      name: new Date(c.submittedAt).toLocaleDateString('pt-BR'),
      score: c.generalAverage,
      student: students.find(s => s.id === c.studentId)?.name || 'Unknown',
    }));
    
  const totalCorrections = new Set(corrections.map(c => `${c.studentId}-${c.simuladoId}`)).size;


  return (
    <div>
      <h1 className="text-3xl font-bold mb-6 text-gray-800 dark:text-gray-200">Início</h1>
        ...
        <div onClick={() => setActivePage('simulados')} className="cursor-pointer hover:scale-105 transition-transform">
            <Card>
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Total de Provas</p>
            ...
            <Card>
              <div className="flex items-center">
                <div className="p-3 bg-indigo-100 dark:bg-indigo-900 rounded-full">
                    <EditIcon className="w-6 h-6 text-indigo-500 dark:text-indigo-300" />
                </div>
                <div className="ml-4">
                  <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Correções Realizadas</p>
                  <p className="text-2xl font-bold text-gray-800 dark:text-gray-200">{totalCorrections}</p>
                </div>
              </div>
            </Card>
        </div>
      ...
      <h2 className="text-2xl font-bold mb-4 text-gray-800 dark:text-gray-200">Histórico de Médias Gerais</h2>
      <Card>
        {corrections.length > 0 ? (
          <ResponsiveContainer width="100%" height={400}>
            <LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
              ...
              />
              <Legend />
              <Line type="monotone" dataKey="score" stroke="#3b82f6" strokeWidth={2} activeDot={{ r: 8 }} name="Média"/>
            </LineChart>
          </ResponsiveContainer>
        ) : (
            <div className="text-center py-12 text-gray-500 dark:text-gray-400">
                <p>Nenhuma correção foi realizada ainda.</p>
                <p className="text-sm">Vá para 'Provas Objetivas' para lançar as primeiras notas.</p>
            </div>
        )}
      </Card>
    </div>
