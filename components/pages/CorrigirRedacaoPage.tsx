...
import { correcaoRedacaoService } from '../../services/supabaseService';
...
export const CorrigirRedacaoPage: React.FC<{
  ...
  showToast: (message: string, type: 'success' | 'error') => void;
    ...
    const saveCorrection = async () => {
        ...
        try {
            const newCorrectionData: Omit<CorrecaoRedacao, 'id'> = {
            ...
            const newCorrection = await correcaoRedacaoService.createCorrecaoRedacao(newCorrectionData);
            setCorrecoesRedacao(prev => [...prev, newCorrection]);
            showToast('Correção salva com sucesso!', 'success');
        } catch (error) {
            console.error('Error saving correction:', error);
            showToast('Erro ao salvar correção. Tente novamente.', 'error');
        } finally {
            resetForm();
        }
    };
