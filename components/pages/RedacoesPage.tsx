...
import { redacaoService } from '../../services/supabaseService';
...
export const RedacoesPage: React.FC<RedacoesPageProps> = ({ redacoes, setRedacoes, showToast }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingRedacao) {
        const updatedRedacao = await redacaoService.updateRedacao(editingRedacao.id, { title: title.trim(), prompt: prompt.trim() });
        setRedacoes(redacoes.map(r => r.id === editingRedacao.id ? updatedRedacao : r));
        showToast('Tema atualizado com sucesso!', 'success');
      } else {
        const newRedacao = await redacaoService.createRedacao({ title: title.trim(), prompt: prompt.trim() });
  ...
  const handleDelete = async (redacaoId: string) => {
    if (window.confirm('Tem certeza que deseja excluir este tema de redação?')) {
      try {
        await redacaoService.deleteRedacao(redacaoId);
        ...
        showToast('Erro ao excluir tema. Tente novamente.', 'error');
      }
    }
  };
