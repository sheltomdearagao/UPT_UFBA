...
import { simuladoService } from '../../services/supabaseService';
...
export const SimuladosPage: React.FC<SimuladosPageProps> = ({ simulados, setSimulados, showToast, setActivePage }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingSimulado) {
        const updatedSimulado = await simuladoService.updateSimulado(editingSimulado.id, { name: simuladoName.trim(), answerKey });
        setSimulados(simulados.map(s => s.id === editingSimulado.id ? updatedSimulado : s));
        showToast('Prova objetiva atualizada com sucesso!', 'success');
      } else {
        const newSimulado = await simuladoService.createSimulado({ name: simuladoName.trim(), answerKey });
      ...
      showToast('Erro ao salvar prova objetiva. Tente novamente.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (simuladoId: string) => {
    if (window.confirm('Tem certeza que deseja excluir esta prova objetiva?')) {
      try {
        await simuladoService.deleteSimulado(simuladoId);
        ...
        showToast('Erro ao excluir prova objetiva. Tente novamente.', 'error');
      }
    }
  };
