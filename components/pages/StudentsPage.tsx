  ...
  const handleSave = async () => {
    ...
    try {
      if (editingStudent) {
        ...
        const updatedStudent = await studentService.updateStudent(editingStudent.id, { name: studentName.trim() });
        setStudents(students.map(s => s.id === editingStudent.id ? updatedStudent : s));
        showToast('Aluno atualizado com sucesso!', 'success');
      } else {
        // Adding new student
        const newStudent = await studentService.createStudent({ name: studentName.trim() });
