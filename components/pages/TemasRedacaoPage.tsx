...
interface TemasRedacaoPageProps {
  temasRedacao: TemaRedacao[];
  setTemasRedacao: React.Dispatch<React.SetStateAction<TemaRedacao[]>>;
  showToast: (message: string, type: 'success' | 'error') => void;
}

export const TemasRedacaoPage: React.FC<TemasRedacaoPageProps> = ({ temasRedacao, setTemasRedacao, showToast }) => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [title, setTitle] = useState('');
  const [prompt, setPrompt] = useState('');
  const [editingTema, setEditingTema] = useState<TemaRedacao | null>(null);
  ...
  const openModalForEdit = (tema: TemaRedacao) => {
    setEditingTema(tema);
    setTitle(tema.title);
    setPrompt(tema.prompt);
    setIsModalOpen(true);
  };
  ...
  const handleSave = () => {
    ...
    if (editingTema) {
      setTemasRedacao(temasRedacao.map(r => r.id === editingTema.id ? { ...r, title: title.trim(), prompt: prompt.trim() } : r));
      showToast('Tema atualizado com sucesso!', 'success');
    } else {
      const newTema: TemaRedacao = {
        id: new Date().toISOString(),
        title: title.trim(),
        prompt: prompt.trim(),
      };
      setTemasRedacao([...temasRedacao, newTema]);
  ...
  const handleDelete = (temaId: string) => {
    if (window.confirm('Tem certeza que deseja excluir este tema de redação?')) {
        setTemasRedacao(temasRedacao.filter(r => r.id !== temaId));
  ...
  return (
    <div>
            ...
            {temasRedacao.map(tema => (
              <li key={tema.id} className="py-4 flex justify-between items-center">
                 <div className="flex items-center">
                    ...
                    <div>
                        <p className="text-lg font-medium text-gray-900 dark:text-gray-100">{tema.title}</p>
                        <p className="text-sm text-gray-500 dark:text-gray-400 truncate max-w-lg">{tema.prompt}</p>
                    </div>
                </div>
                <div className="space-x-2">
                    <button onClick={() => openModalForEdit(tema)} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-600 rounded-full" aria-label="Edit redacao">
                        <EditIcon className="w-5 h-5 text-gray-500 dark:text-gray-400" />
                    </button>
                    <button onClick={() => handleDelete(tema.id)} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-600 rounded-full" aria-label="Delete redacao">
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingTema ? "Editar Tema" : "Adicionar Novo Tema"}>
